/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   merge_redir_tokens.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hocsong <hocsong@student.42seoul.kr>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/16 14:47:18 by hocsong           #+#    #+#             */
/*   Updated: 2023/02/16 15:48:52 by hocsong          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"
#include "constants.h"

static void	merge_redir_token(t_str *str, int idx);
static int	get_redir_in_s(t_str *str, int redir_order, int optype);
static void	merge(char **words, int idx);
static int	should_merge(t_str *str, int idx, int optype);

void	merge_redir_tokens(t_str *str)
{
	int	i;

	i = 0;
	while (str -> words[i])
	{
		merge_redir_token(str -> words, i);
		i++;
	}
}

static void	merge_redir_token(t_str *str, int idx)
{
	if (get_token_type(str -> words, idx) == REDIR_IN \
		&& get_token_type(str -> words, idx + 1) == REDIR_IN \
		&& should_merge(str, idx, REDIR_IN))
		merge(str, idx);
	if (get_token_type(str -> words, idx) == REDIR_OUT \
		&& get_token_type(str -> words, idx + 1) == REDIR_IN \
		&& should_merge(str, idx, REDIR_OUT))
		merge(str, idx);
}

static int	should_merge(t_str *str, int idx, int optype)
{
	int	i;
	int	count;

	i = 0;
	count = 0;
	while (str -> words[i] && i <= idx)
	{
		if (str -> words[i] == optype)
			count++;
		i++;
	}
	return (get_redir_in_s(str, count, optype));
}

static int	get_redir_in_s(t_str *str, int redir_order, int optype)
{
	int	i;
	int	count;

	i = 0;
	count = 0;
	while (str -> s[i] && count < redir_order)
	{
		if (get_token_type(str -> s[i]) == optype)
			count++;
		i++;
	}
	if (str -> s[i] == optype)
		return (1);
	return (0);
}

static void	merge(char **words, int idx)
{
	words[idx] = ft_strjoin(words[idx], words[idx + 1]);
	idx++;
	while (words[idx])
	{
		words[idx] = words[idx + 1];
		idx++;
	}
	words[idx] = 0;
}
